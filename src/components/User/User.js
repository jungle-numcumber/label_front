...
export default function User() {
  let onOffHistory = useSelector(state => {
    return state.onOffHistory;
  });
...
const UserProfile = () => {
  ...
  return <UserProfileShow result={result} length={result.length}></UserProfileShow>;
};
...
const Tr = () => {
  ...
  return <BookList result={result} length={result.length}></BookList>;
};
      ...
      result.push(
        <tr key={i}>
          ...
          <td className={styles.td}>
            {props.result[i].recentlyReadPage} / {props.result[i].totalPage}
          </td>
      ...
      for (let i = 1; i < 365; i++) {
        if (props.date[0].commitGrass[i] !== '0') {
          result.push(
            <li
              key={i}
              date={subtractDays(-i + 364, new Date())}
              data-level={1}
              onClick={() => {
