  ...
  useEffect(() => {
    if (readOnly === -1) {
      const getPageLink = async () => {
        const pageLink = await axios
          .get(`https://inkyuoh.shop/pdfs/${pdfIdx}/pages/${currentPageNumber}`)
          .then(response => {
        ...
        const getHighlightData = await axios
          .get(`https://inkyuoh.shop/highlights/pdfs/${pdfIdx}/pages/${currentPageNumber}`)
          .then(response => {
    ...
    if (readOnly === 1) {
      const getPageLink = async () => {
        ...
        const getHighlightData = await axios
          .get(`https://inkyuoh.shop/highlights/pages/${currentPageNumber}/commitIdx/${commitIdx}`)
          .then(response => {
            setHighlightData(response.data.result);
          });
      };
  ...
  useEffect(() => {
    ...
    if (commitIdx !== -1) {
      const getMarkdownValue = async () => {
        axios
          .post('https://inkyuoh.shop/commits/books/2/2', {
            commitIdx: commitIdx,
          })
          .then(response => {
    ...
    <main className="PersonalReading">
      {readOnly === 1 ? (
        <div className="ReadOnlyText">
          읽기 전용 상태입니다.&nbsp;&nbsp;&nbsp;<FontAwesomeIcon icon={faLock}></FontAwesomeIcon>
        </div>
      ) : null}
      ...
      <div className="PersonalReading__container" style={loading === true ? { opacity: 0.8 } : null}>
        <aside className="PersonalReading__sideBar">
          <SideBar
            ...
            commitIdx={commitIdx}
            setCommitIdx={setCommitIdx}
            currentBookInfo={currentBookInfo}
          ></SideBar>
        </aside>
        ...
        <div className="PersonalReading__mainPage" style={props.mode === true ? { flex: 3 } : { flex: 1 }}>
          {props.mode === true ? (
            <article
              ...
              style={props.mode === true ? { flex: 2 } : { flex: 0 }}
            >
                  ...
                  <input
                    ...
                    onKeyUp={event => {
                      ...
                      if (
                        ...
                        event.target.value <= currentBookInfo.totalPage
                      ) {
          ...
          <aside
            ...
            style={props.mode === true ? { flex: 1 } : { flex: 1 }}
          >
            <HighlightList
              readOnly={readOnly}
              forceUpdate={forceUpdate}
              ...
              setCommitIdx={setCommitIdx}
              pdfIdx={pdfIdx}
              totalPage={currentBookInfo.totalPage}
              currentPageNumber={currentPageNumber}
              updateHighlightList={updateHighlightList}
              setUpdateHighlightList={setUpdateHighlightList}
            ></HighlightList>
            {props.mode === false ? (
